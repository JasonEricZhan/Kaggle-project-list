...
#corpus_twitter= parallelize_dataframe(corpus_raw, multiply_columns_glove_preprocess)
#corpus_twitter=parallelize_dataframe(corpus_twitter, multiply_columns_clean)
...
def get_model():
   ...
   # embedded_sequences_2= Embedding(len(char2index),100,trainable=True)(Ngram_input)
    ...
    #x_att_2_2 =Attention()(x_gru_2)
    #x_att_2= Dropout(0.3)(x_att_2)
    #x_att_2_2= Dropout(0.3)(x_att_2_2)
    #l_dense_2 = TimeDistributed(Dense(128))(x_lstm_2)
    #activation='elu'
    #l_dense_1 =TimeDistributed(Dense(160))(x_gru_1)
    #x_dense= concatenate([x_max,x_ave])
    #x_dense_2= concatenate([x_max_2,x_ave_2])
    
    #x_att_3 =AttentionWithContext()(x_gru_1)
    #x_att_2= Dropout(0.325)(x_att_2)
    ...
    #x= Dropout(0.45)(x)
    #x=BatchNormalization()(x)
    #x_dense = Dense(90, activation="elu")(x_att_1)
    x=capsule
    
    #x_dense_2 = Dense(84, activation="elu")(x_dense_2)
    #x= LeakyReLU(alpha=.001)(x)
    #x_dense = Dropout(0.2)(x_dense)
    #x_dense_2 = Dropout(0.15)(x_dense_2)
    #x=BatchNormalization()(x)
    ##x=BatchNormalization()(x)
    #x = concatenate([x_dense,x_att_2])
    #x_2 = concatenate([x_dense_2,x_att_2_2])
    #x=concatenate([x_2,x])
    #x=Add()([x_dense,x_att_2])
    #x=BatchNormalization()(x)
    #x = Dropout(0.2)(x)
...
accumulator=[]
#pred_val_accumulator=pickle.load(open("prediction_RNN_Capsule_fold_"+str(6)+".pkl", "rb")) 
#test_accumulator=pickle.load(open("test_average__Capsule_fold_"+str(6)+".pkl", "rb"))
...
for i in range(split):
    ...
    c_val_y = y_tr[train.id.isin(val_ids[i])]
        ...
        #c_train_X_2=np.vstack((c_train_XOne_2,c_train_XTwo_2))
   
    #if  count==0:
        #continue
    i=count
    file_path="weights_base_5_fold_Capsule.hdf5"
    ...
    #pred_val = model.predict({'main_input':c_val_X, 'aux_input': c_val_X_2},batch_size=1024, verbose=1)
    ...
    pickle.dump(pred_val_accumulator,open("prediction_RNN_Capsule_fold_"+str(i)+".pkl", "wb"))
    pickle.dump(test_accumulator,open("test_average__Capsule_fold_"+str(i)+".pkl", "wb"))
...
pickle.dump(pred_val_accumulator,open("prediction_RNN_Capsule.pkl", "wb"))
if test_accumulator is not None:
   pickle.dump(test_average,open("test_average_Capsule.pkl", "wb"))
...
#file_path="weights_base_256_GRU.hdf5"
file_path="weights_base_256_Capsule.hdf5"
checkpoint = ModelCheckpoint(file_path, monitor='val_loss', verbose=1, save_best_only=True, mode='min')
...
history=model.fit( X_tr_1,y_tr, batch_size=batch_size, epochs=epochs, validation_split=0.07, callbacks=callbacks_list)
"""
import matplotlib
matplotlib.use('Agg')
import matplotlib.pyplot as plt
...
sample_submission.to_csv("baseline_capsule.csv", index=False)