...
#model_ted = Word2Vec(sentences=corpus_raw, size=100, window=100, min_count=50, workers=2000, sg=0)  

#weights=model_ted.wv.syn0

#np.save(open("self_train_weight.npz", 'wb'), weights)

#weights=np.load(open("self_train_weight.npz", 'rb'))
...
#print('Found %s word vectors of fastText' % len(model.wv))
...
embedding_matrix_2 = np.zeros((nb_words, embed_size))
for word, i in tokenizer.word_index.items():
    if i >= nb_words:
        continue
    embedding_vector = embeddings_index.get(word)
...
def get_model():
    ...
    #embedded_sequences_1_2= Embedding(max_features, embed_size,weights=[embedding_matrix_2],trainable=False)(main_input)
    ...
    hidden_dim=136
    #embedded_sequences = concatenate([embedded_sequences,embedded_sequences_1_2])
    ...
    #hidden_dim=30
    #x_2=SpatialDropout1D(0.1)(embedded_sequences_2)                    #0.1
    #x_gru_2 = Bidirectional(CuDNNGRU(hidden_dim,recurrent_regularizer=regularizers.l2(1e-6),return_sequences=True))(x_2)
    
    #x_gru_1 = concatenate([x_gru_1,x_gru_2])
    ...
    #x_gru_1 = TimeDistributed(Dense(168))(x_gru_1)
    x_ave=GlobalAveragePooling1D()(x_gru_1)
    #x_ave= Dropout(0.3)(x_ave)
    #x_ave_2=GlobalAveragePooling1D()(x_gru_2)
    #x_ave_2= Dropout(0.15)(x_ave_2)
    #x_ave= concatenate([x_ave,x_ave_2])
    ...
    #x_max_2= Dropout(0.15)(x_max_2)
    #x_max= concatenate([x_max,x_max_2])
    #x_max= Dropout(0.25)(x_max)
    #x_max= Dropout(0.3)(x_max)
    #x_att_2 =Attention()(x_gru_1)
    #x_att_2_2 =Attention()(x_gru_2)
    #x_att_2= concatenate([x_att_2,x_att_2_2])
...
pred_val_accumulator=pickle.load(open("prediction_GRU_no_char"+str(7)+".pkl", "rb")) 
test_accumulator=pickle.load(open("test_average__GRU_no_char"+str(7)+".pkl", "rb"))
from sklearn.metrics import roc_auc_score
K_fold=10
